...
A pane with todos shows a `▾` indicator with its outstanding count at the right end of its top border; a pane whose todos are all done shows a dimmed `▾`, and a pane with none shows nothing. `prefix+ctrl+t`, or a click on the indicator, opens that pane's todo panel. Inside it, `Up`/`Down` or `j`/`k` move the selection, `Enter` (or a row click) opens the todo for editing, `Space` toggles done, `g` follows a linked todo to its pane, `d` removes the selected todo, `c` clears the completed ones, and `Esc` or `q` closes. A todo linked to a pane that no longer exists renders dimmed and does nothing when followed. In the edit view, `Tab` cycles the priority, `ctrl+l` cycles the link between keep, none, and each linkable pane, `ctrl+d` toggles done (the panel's `Space`, which the edit view's text field owns), `Enter` saves, and `Esc` cancels. Clicking the priority, link, or done row does the same as its shortcut. The done row is absent while composing a new todo, which is never already done. `keys.add_pane_todo` is unset by default and opens the edit view on a new todo. Set `ui.show_pane_todo_indicator = false` to hide the indicator, and `ui.pane_todo_color` to override its color.
