...
The notification center (`prefix+ctrl+n`, or click the `◆` indicator at the top right of the tab bar) lists recent notifications newest-first and marks them all seen on open. With `ui.notification_center_position = "bottom-right"` the indicator floats in the frame's bottom-right corner instead, and the panel opens above it. Inside it, `Up`/`Down` or `j`/`k` move the selection, `Enter` jumps to the selected notification's pane and closes, `Esc` or `q` closes, and clicking a row jumps to that row's pane. `c` or the "Clear all" button at the bottom empties the log. Notifications without a pane target are not actionable.
