...
A range can also continue past nine with letters, so lists longer than nine entries stay reachable:
...
Entries 1–9 take the digits, then the letters continue from entry 10.

On **macOS**, the letters `e`, `i`, `n`, and `u` are **skipped**. Those are Option dead keys on ABC and most Latin layouts — Option+e produces `´` and Option+i produces a pending `ˆ` rather than `alt+e` or `alt+i`, so the keypress never reaches Herdr. Ghostty's `macos-option-as-alt` does not change this: it defeats the plain Option translation but not dead-key composition. Skipping them means every jump letter Herdr shows is one you can actually press, at the cost of the entry-to-letter mapping differing from Linux and Windows, which keep the full `a..z`. So on macOS entry 14 is `alt+f`, not `alt+e`.
...
An agent that is working only because of work it *launched* — a background shell, a background agent, an MCP task still running — is drawn apart from an agent mid-turn: it pulses slowly between `◇` and `◈` (one swap per eight spinner ticks, so about one and a half seconds at the default rate) instead of spinning, because the agent itself is parked at its prompt and will wake when that work finishes. Both frames are overridable as `background` and `background_alt` in `[ui.state_symbols]`, and the icon and its state text take `[ui.state_colors] background`, which follows `working` until it is set.

The symbols themselves are static. In the sidebar's agent rows the working icon still moves: with `status_spinner = "on"` (the default) every working agent's icon steps through a spinner on one shared tick, `status_spinner_ms` apart (default 200, clamped to 50–2000). The tick is armed only while an agent is working, so an idle session never wakes for it, and the agent's own terminal title is not involved — agents freeze their title spinner when a pane is unfocused, so the title cannot clock the rows that are not focused. Space rows keep the static working glyph. Set `status_spinner = "off"` to keep every working icon static.

The symbols style draws `×` blocked, `◐` working, `□` done (finished, not looked at yet), `✓` idle (finished and seen), and `·` unknown. Either style can be adjusted per state with `[ui.state_symbols]`; unset states keep the style's glyph. A value must be exactly one terminal cell wide so the icon column stays aligned — wider or empty values are reported as a config diagnostic and ignored.
...
- `workspace` — workspace name.
- `branch` — Git branch when available. A checkout on no branch shows where it is instead — `@a620c06`, or `rebase @a620c06` / `bisect @a620c06` while that operation runs — so the row stays; outside a repository the token is empty.
