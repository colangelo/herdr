...
The symbols themselves are static. In the sidebar's agent rows the working icon can still move: with `status_spinner = "agent"` (the default) Herdr steps its own spinner each time the agent's terminal-title spinner ticks, so the motion follows the agent's pace with no timer of its own. Claude Code and Codex animate their titles; an agent whose title holds still keeps the static working glyph, and space rows never animate. Set `status_spinner = "off"` to keep every working icon static.
