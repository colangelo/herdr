...
Todos come back in display order: highest priority first (`high`, `normal`,
`low`), outstanding before done, then creation order. Every entry names its own
`pane_id`, so a whole-session listing stays unambiguous. `link_pane_id` and
`link_label` appear only on linked todos. `link_alive` is `false` once the
