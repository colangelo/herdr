...
- add, list, edit, remove, and clear per-pane todos
...
| Notification | `notification.show`, `notification.list`, `notification.mark_seen`, `notification.clear` |
| Todo | `todo.list`, `todo.add`, `todo.update`, `todo.remove`, `todo.clear` |
...
`layout.balance` rebalances every split in a tab so all panes become equally
sized, preserving the tree shape and each split's orientation (like tmux
`even-horizontal`/`even-vertical`). Omit `tab_id` and `pane_id` to target the
active tab. The response is `type: "layout_balanced"` with the updated portable
`layout`.
...
`layout.set_preset` rebuilds a tab into a tmux-style preset: `even_horizontal`
(one equal-width row), `even_vertical` (one equal-height column), or `tiled` (a
near-square grid). Unlike `layout.balance`, this restructures the tree. Omit
`tab_id` and `pane_id` to target the active tab. The response is
`type: "layout_preset_applied"` with the updated portable `layout`.

```json
{"id":"req_preset","method":"layout.set_preset","params":{"tab_id":"w1:t1","preset":"tiled"}}
```
...
`pane.clear` purges the target pane's saved scrollback with tmux
`clear-history` semantics. The visible screen contents and the running process
are untouched, and the call is a safe no-op when the scrollback is already
empty:
...
The response is `type: "ok"`. Unknown pane ids fail with `pane_not_found`.

`pane.respawn` replaces the target pane's process without removing the pane.
The pane keeps its pane id, terminal id, position, size, label, and todos; a
pane launched with a command re-runs that command, and a plain shell pane gets
a fresh shell. The pane's agent runtime identity is cleared so detection
re-identifies whatever comes back:

```json
{"id":"req_respawn","method":"pane.respawn","params":{"pane_id":"w1:p1"}}
```

The response is `type: "ok"`. A pane that still holds live work — a running
child process or outstanding todos — fails the first request with
`confirmation_required` and is left untouched; repeating the request confirms
and respawns, the same confirmation the keybinding applies. Unknown pane ids
fail with `pane_not_found`.
...
The server also keeps a bounded in-memory notification log (the newest 100
entries) of every in-app toast it showed: agent needs-attention and finished
notifications, update notifications, and `notification.show` injections. Each
entry carries a `read` flag, false when posted; the unread count is the
number of unread entries. The log is in-memory only and empties on a cold
server restart.

List the log newest-first:

```json
{"id":"req_nlist","method":"notification.list","params":{}}
```

```json
{"id":"req_nlist","result":{"type":"notification_list","notifications":[{"id":3,"kind":"finished","title":"claude finished","context":"api · 2","workspace_id":"w1","pane_id":"w1:p2","posted_at_unix":1789000000,"read":false}],"unread_count":1}}
```

`kind` is `needs_attention`, `finished`, or `update_installed`. `workspace_id`
and `pane_id` are present when the notification targets a pane; `pane_id` is
omitted when the pane no longer resolves.

Mark one entry read by `id`, or every entry read with no `id` (idempotent
either way):

```json
{"id":"req_nseen","method":"notification.mark_seen","params":{"id":3}}
```

```json
{"id":"req_nseen","result":{"type":"notification_mark_seen","changed":true,"unread_count":0}}
```

Empty the log entirely (the notification center's "Clear all" action):

```json
{"id":"req_nclear","method":"notification.clear","params":{}}
```

```json
{"id":"req_nclear","result":{"type":"notification_cleared","cleared":3}}
```

`cleared` is how many entries were removed. Ids stay monotonic across a clear,
so a later entry never reuses an id.

Activating a notification in the Herdr TUI's notification center marks that
same entry read, so external consumers and the built-in unread indicator
always agree. Subscribe to `notification.posted` for a push feed of new
entries.

Todos are the durable counterpart: each pane owns a list of next steps that
survives a server restart and a live handoff, exposed as `todo.list`,
`todo.add`, `todo.update`, `todo.remove`, and `todo.clear`. `herdr todo` is the
CLI wrapper for the same methods.

List one pane's todos, or every pane's when `pane_id` is omitted:

```json
{"id":"req_tlist","method":"todo.list","params":{"pane_id":"w1:p2"}}
```

```json
{"id":"req_tlist","result":{"type":"todo_list","todos":[{"pane_id":"w1:p2","id":3,"text":"rerun the deploy","done":false,"priority":"high","link_pane_id":"w1:p1","link_label":"infra","link_alive":true,"created_at_unix":1789000000,"updated_at_unix":1789000100}]}}
```

Todos come back in display order: outstanding before done, then highest
priority first (`high`, `normal`, `low`), then creation order. Every entry names
its own `pane_id`, so a whole-session listing stays unambiguous. `link_pane_id`
and `link_label` appear only on linked todos. `link_alive` is `false` once the
linked pane is gone; the label is kept so a dead link can still say what it
meant, and it never resolves to some other pane.

Add a todo to a pane:

```json
{"id":"req_tadd","method":"todo.add","params":{"pane_id":"w1:p2","text":"rerun the deploy","priority":"high","link_pane_id":"w1:p1"}}
```

```json
{"id":"req_tadd","result":{"type":"todo","todo":{"pane_id":"w1:p2","id":3,"text":"rerun the deploy","done":false,"priority":"high","link_pane_id":"w1:p1","link_label":"infra","link_alive":true,"created_at_unix":1789000000,"updated_at_unix":1789000000}}}
```

`priority` is optional and defaults to `normal`. `link_pane_id` is optional and
must resolve to a live pane; the link label is captured from that pane's label,
its agent name, or the id you passed, in that order.
Text is trimmed and must not be empty, a todo is capped at 500 characters, and
a pane is capped at 50 todos. Nothing is silently truncated: over-limit or
malformed writes fail with `todo_text_empty`, `todo_text_too_long`,
`todo_limit_reached`, `todo_link_unresolved`, or `pane_not_found`.

Update one todo. Omitted fields keep their current value, and `clear_link`
removes the link and wins over `link_pane_id`:

```json
{"id":"req_tupd","method":"todo.update","params":{"pane_id":"w1:p2","id":3,"done":true}}
{"id":"req_tunlink","method":"todo.update","params":{"pane_id":"w1:p2","id":3,"text":"rerun the deploy after review","priority":"normal","clear_link":true}}
```

The response is `type: "todo"` with the stored record. An id that is not on
that pane fails with `todo_not_found`, and a rejected update leaves the todo
untouched.

Remove one todo, or clear a pane's list; `done_only` keeps the outstanding ones:

```json
{"id":"req_trm","method":"todo.remove","params":{"pane_id":"w1:p2","id":3}}
{"id":"req_tclear","method":"todo.clear","params":{"pane_id":"w1:p2","done_only":true}}
```

```json
{"id":"req_trm","result":{"type":"ok"}}
{"id":"req_tclear","result":{"type":"todo_cleared","removed":2}}
```

Every call that changes a pane's list emits `todo.changed` for that pane; a
clear that removed nothing changes nothing and emits nothing.
...
Notification subscriptions include `notification.posted`, emitted whenever an
entry is appended to the server's notification log. The event carries the same
`notification` record as `notification.list`.

Todo subscriptions include `todo.changed`, emitted once per `todo.*` call that
changed a pane's list. The event carries only the affected `pane_id`; read the
list back with `todo.list` so consumers always see one shape.
