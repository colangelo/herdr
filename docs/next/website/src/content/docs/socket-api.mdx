...
notifications, update notifications, and `notification.show` injections. The
log carries one `last_seen_id` marker; entries newer than the marker are
unread. The log is in-memory only and empties on a cold server restart.
...
```json
{"id":"req_nlist","result":{"type":"notification_list","notifications":[{"id":3,"kind":"finished","title":"claude finished","context":"api · 2","workspace_id":"w1","pane_id":"w1:p2","posted_at_unix":1789000000}],"unread_count":1,"last_seen_id":2}}
```
...
Advance the seen marker to the newest entry (idempotent):

```json
{"id":"req_nseen","method":"notification.mark_seen","params":{}}
```

```json
{"id":"req_nseen","result":{"type":"notification_mark_seen","last_seen_id":3,"changed":true}}
```
...
Opening the notification center panel in the Herdr TUI advances the same
marker, so external consumers and the built-in unread indicator always agree.
Subscribe to `notification.posted` for a push feed of new entries.
