...
`notification list` prints the server's notification log newest-first — the same feed as the in-app notification center — with unread entries marked `*` and a trailing unread count. Pass `--json` for the raw `notification.list` response, including each entry's `id`, `kind`, target ids, and `posted_at_unix`. The log holds the newest 100 entries in server memory and empties on a cold server restart. `notification clear` empties the log (the same action as the notification center's "Clear all" button) and reports how many entries were removed.
