---
title: CLI reference
description: Herdr commands for sessions, workspaces, tabs, panes, notifications, todos, agents, waits, integrations, and status.
---
...
```bash
herdr notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right|center] [--sound none|done|request]
herdr notification list [--json]
herdr notification clear [--json]
```
...
`notification list` prints the server's notification log newest-first — the same feed as the in-app notification center — with unread entries marked `*` and a trailing unread count. An entry reads as unread until it is activated in the notification center or marked via `notification.mark_seen`. Pass `--json` for the raw `notification.list` response, including each entry's `id`, `kind`, `read` flag, target ids, and `posted_at_unix`. The log holds the newest 100 entries in server memory and empties on a cold server restart. `notification clear` empties the log (the same action as the notification center's "Clear all" button) and reports how many entries were removed.

## Todos

```bash
herdr todo add <text> [--pane ID|--current] [--priority high|normal|low] [--link <target>] [--json]
herdr todo list [--pane ID|--current|--all] [--json]
herdr todo done <id> [--pane ID|--current] [--json]
herdr todo undone <id> [--pane ID|--current] [--json]
herdr todo edit <id> [--text TEXT] [--priority high|normal|low] [--link <target>|--unlink] [--pane ID|--current] [--json]
herdr todo rm <id> [--pane ID|--current] [--json]
herdr todo clear [--done] [--pane ID|--current] [--json]
```

Every pane carries its own todo list. Without `--pane` or `--current`, each verb acts on the calling pane's `HERDR_PANE_ID`, so an agent about to exit can record its next steps with `herdr todo add "hand back to review"` without knowing its own pane id. Run outside a Herdr pane without a target and the command fails instead of guessing; `todo list` also accepts `--all` to list every pane in the session.

`--priority` is `high`, `normal`, or `low` and defaults to `normal`. `--link <target>` points a todo at another pane by pane id (`w1:p2`); the link captures a label for the target when it is made — that pane's label, its agent name, or the id you passed — so it still reads sensibly once the target is gone. `edit --unlink` drops the link. A pane holds at most 50 todos of at most 500 characters each; writes past either limit are rejected with `todo_limit_reached` or `todo_text_too_long` rather than truncated.

`todo list` prints one todo per line in display order — outstanding before done, then highest priority first, then creation order — marking outstanding todos with `*`, followed by the id, priority, and text, with ` → label` appended for a linked todo and ` (gone)` when its target pane no longer exists. A trailing line counts the outstanding todos when any remain. `--all` adds a pane column. Pass `--json` for the raw `todo.list` response, including each todo's `link_pane_id`, `link_alive`, `created_at_unix`, and `updated_at_unix`.

Todos belong to the pane and follow it: they ride the session snapshot across server restarts and `herdr update --handoff`, and links are remapped to the restored pane ids. A link whose target did not come back keeps its label and stops resolving instead of pointing at some other pane.
...
herdr pane clear [<pane_id>|--pane ID|--current]
herdr pane close <pane_id>
herdr pane respawn <pane_id>
```

`pane respawn` restarts a pane's process in place. The pane keeps its ID,
position, size, label, and todos; only the process is replaced. A pane launched
with a command re-runs that command, and a plain shell pane gets a fresh shell.
The replacement starts in the pane's current directory, not the one the pane was
launched in, and the respawn never moves focus.
When the pane still holds live work — a running child process or outstanding
todos — the first request returns `confirmation_required` and changes nothing;
repeating it confirms and respawns. Use it to recover a wedged agent or dev
server without losing the pane's place in the layout.
...
```bash
herdr pane send-text <pane_id> <text> [--chunk BYTES [--chunk-delay MS]]
...
`send-text` delivers the text as one raw write, without bracketed paste. Some TUIs, Claude Code among them, classify input by the size of the burst: a single write of about 1000 bytes or more is collapsed into a `[Pasted text #1]` placeholder, and a command such as `/compact` never expands it. `--chunk BYTES` splits the text into pieces of at most that many bytes, never cutting a UTF-8 character, and sends each as its own write with `--chunk-delay MS` (default 20) between them, so the pieces arrive as typed text. The command returns after the last piece is sent, so you can read the pane or press Enter right after. `--chunk 300` is a safe size for Claude Code; put `--` before text that itself starts with `--chunk`.
