...
todo は永続する対になる仕組みです。各ペインが次の一手のリストを持ち、サーバー再起動やライブハンドオフをまたいで残ります。メソッドは `todo.list`、`todo.add`、`todo.update`、`todo.remove`、`todo.clear` で、`herdr todo` は同じメソッドの CLI ラッパーです。

1 つのペインの todo を一覧します。`pane_id` を省略するとすべてのペインの todo を返します:
...
todo は表示順 (未完了が先 → 優先度の高い順 `high`、`normal`、`low` → 作成順) で返ります。各エントリは自分の `pane_id` を持つため、セッション全体の一覧でも曖昧になりません。`link_pane_id` と `link_label` はリンク付きの todo にのみ現れます。リンク先のペインがなくなると `link_alive` は `false` になり、ラベルは残るので切れたリンクでも意味が読み取れます。別のペインに解決されることはありません。
...
`priority` は任意でデフォルトは `normal` です。`link_pane_id` は任意で、実在するペインに解決される必要があります。リンクラベルは、そのペインのラベル、エージェント名、渡した ID の順に取り込まれます。テキストは前後の空白が除去され、空にはできません。1 件あたり 500 文字、1 ペインあたり 50 件が上限です。黙って切り詰めることはなく、上限超過や不正な書き込みは `todo_text_empty`、`todo_text_too_long`、`todo_limit_reached`、`todo_link_unresolved`、`pane_not_found` で失敗します。

1 件の todo を更新します。省略したフィールドは現在の値を保ち、`clear_link` はリンクを外し `link_pane_id` より優先されます:
...
レスポンスは `type: "todo"` で、保存されたレコードを含みます。そのペインに存在しない id は `todo_not_found` で失敗し、拒否された更新は todo をそのまま残します。

1 件の todo を削除するか、ペインのリストを一括で消します。`done_only` は未完了の todo を残します:
...
ペインのリストを変更した呼び出しは、そのペインについて `todo.changed` を発行します。何も削除しなかった clear は何も変更しないため、イベントも発行しません。
...
todo のイベント購読には `todo.changed` があります。ペインのリストを変更した `todo.*` の呼び出しごとに 1 回発行され、対象の `pane_id` だけを運びます。リストは `todo.list` で読み直してください。そうすればコンシューマーが見る形は常に 1 つです。
