...
todo は表示順 (優先度の高い順 `high`、`normal`、`low` → 未完了が先 → 作成順) で返ります。各エントリは自分の `pane_id` を持つため、セッション全体の一覧でも曖昧になりません。`link_pane_id` と `link_label` はリンク付きの todo にのみ現れます。リンク先のペインがなくなると `link_alive` は `false` になり、ラベルは残るので切れたリンクでも意味が読み取れます。別のペインに解決されることはありません。
