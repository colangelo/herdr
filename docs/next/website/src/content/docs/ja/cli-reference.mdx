...
`todo list` は表示順 (未完了が先 → 優先度の高い順 → 作成順) に 1 行ずつ出力します。未完了の todo には `*` が付き、続いて id、優先度、テキストが並びます。リンク付きの todo には ` → ラベル` が、対象ペインがすでに存在しない場合はさらに ` (gone)` が付きます。未完了が残っている場合は、最後の行に未完了件数が出ます。`--all` はペイン列を追加します。生の `todo.list` レスポンス (各 todo の `link_pane_id`、`link_alive`、`created_at_unix`、`updated_at_unix` を含む) が欲しいときは `--json` を渡してください。
