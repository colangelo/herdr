...
The sidebar rolls state upward. A row covering several panes — a workspace, a tab, or a collapsed worktree group — shows the state of the most significant agent under it, ranked `blocked`, `working`, `done`, `idle`, `unknown`.

So a blocked agent makes its pane, tab, and workspace look blocked, and a workspace holding a working agent reads as working even when another agent beside it has already finished. A done agent stays visible until you view it.

A rolled-up row reflects only the panes it covers. Viewing one pane never changes what a row shows for the others.

Sorting is ranked differently, on purpose. Under `agent_panel_sort = "priority"` or `workspace_sort = "priority"`, a finished agent sorts *above* a working one, because it is the one waiting on you. The top of a priority-sorted list is what wants a decision; the state on each row is what its agents are actually doing.

The `agent_status` that `workspace.info` and `tab.info` report follows the same rollup as the sidebar row. Per-pane `agent_status` is that pane's own state, with no rollup.
