...
`todo list` 按展示顺序(优先级从高到低 → 未完成在前 → 创建顺序)逐行输出。未完成的待办以 `*` 标记,后面是 id、优先级和文本;带链接的待办会追加 ` → 标签`,目标窗格已不存在时再追加 ` (gone)`。还有未完成的待办时,最后一行给出未完成的数量。`--all` 会多出一列窗格 ID。需要原始的 `todo.list` 响应(含每条待办的 `link_pane_id`、`link_alive`、`created_at_unix` 和 `updated_at_unix`)时加 `--json`。
