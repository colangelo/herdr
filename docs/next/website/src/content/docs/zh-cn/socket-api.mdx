...
| 待办 | `todo.list`、`todo.add`、`todo.update`、`todo.remove`、`todo.clear` |
...
待办是与通知相对的持久机制:每个窗格拥有一份下一步清单,跨服务器重启和实时交接保留,方法为 `todo.list`、`todo.add`、`todo.update`、`todo.remove` 和 `todo.clear`。`herdr todo` 是同一批方法的 CLI 包装。

列出一个窗格的待办;省略 `pane_id` 时返回所有窗格的待办:
...
待办按展示顺序返回:未完成在前 → 优先级从高到低(`high`、`normal`、`low`)→ 创建顺序。每条都带有自己的 `pane_id`,因此整个会话的列表也不会有歧义。`link_pane_id` 和 `link_label` 只出现在带链接的待办上。链接的窗格消失后 `link_alive` 变为 `false`,标签仍然保留,所以失效的链接仍读得懂,并且永远不会解析到别的窗格。
...
`priority` 可选,默认 `normal`。`link_pane_id` 可选,必须解析到一个存在的窗格;链接标签依次取自该窗格的标签、智能体名称,或你传入的 ID。文本会被去除首尾空白且不能为空,每条待办上限 500 个字符,每个窗格上限 50 条。不会静默截断:超限或非法的写入会以 `todo_text_empty`、`todo_text_too_long`、`todo_limit_reached`、`todo_link_unresolved` 或 `pane_not_found` 失败。

更新一条待办。省略的字段保持原值,`clear_link` 解除链接并优先于 `link_pane_id`:
...
响应为 `type: "todo"`,携带保存后的记录。不属于该窗格的 id 会以 `todo_not_found` 失败,被拒绝的更新不会改动待办。

删除一条待办,或清空一个窗格的列表;`done_only` 保留未完成的待办:
...
每次改变窗格列表的调用都会为该窗格发出 `todo.changed`;没有删除任何内容的 clear 不改变任何状态,也不会发出事件。
...
待办事件订阅包括 `todo.changed`,每次改变窗格列表的 `todo.*` 调用发出一次。事件只携带受影响的 `pane_id`;用 `todo.list` 把列表读回来,消费者看到的结构就始终一致。
