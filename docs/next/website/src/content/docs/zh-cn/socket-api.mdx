...
待办按展示顺序返回:优先级从高到低(`high`、`normal`、`low`)→ 未完成在前 → 创建顺序。每条都带有自己的 `pane_id`,因此整个会话的列表也不会有歧义。`link_pane_id` 和 `link_label` 只出现在带链接的待办上。链接的窗格消失后 `link_alive` 变为 `false`,标签仍然保留,所以失效的链接仍读得懂,并且永远不会解析到别的窗格。
