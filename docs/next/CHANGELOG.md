...
- The todo board is now titled `todos/notes`. What a pane records is as often a note to self as a task, and a title naming only one of them says the other does not belong there. Only the title changes: `herdr todo`, `keys.open_todo_board`, the socket API and the protocol keep their names, because those are addresses typed into scripts and configs rather than words the user reads while working.
