...
- Settings and `ui.status_indicators = "symbols"` can now use distinct static shapes for blocked, working, done, idle, and unknown agent states: `×` blocked, `◐` working, `□` done and not yet seen, `✓` idle and seen, `·` unknown. (#2260)
- `[ui.state_symbols]` overrides the glyph drawn for any agent state on top of the chosen `ui.status_indicators` style; values must be one terminal cell wide and apply on config reload.
- An agent that is working only because of work it launched — a background shell, a background agent, an MCP task still running — now pulses slowly between `◇` and `◈` in the sidebar instead of spinning like an agent mid-turn, since the agent itself is parked at its prompt and wakes when that work finishes. Both frames are overridable as `background` / `background_alt` in `[ui.state_symbols]`, and the icon and its state text take their own `[ui.state_colors] background`, which follows `working` until it is set.
- The working icon in sidebar agent rows animates again: every working agent steps through a spinner on one slow shared tick (`ui.status_spinner_ms`, default 200 ms) that is armed only while an agent is working, so an idle session costs nothing; `ui.status_spinner = "off"` keeps it static. The icon is not clocked from the agent's terminal title, because agents freeze that spinner for unfocused panes.
...
- Pane todos are now visible and editable in the TUI: every pane that draws a top border shows a `▾` indicator at the right end of it, carrying the outstanding count coloured by the highest priority among them, a bare `▾` once everything is done, and the same `▾` dimmed further when the pane has no todos at all — so the control sits in the same place on every pane and an empty pane can be opened by mouse. Clicking it or pressing `keys.open_pane_todos` (`prefix+ctrl+t`) opens a panel anchored to that pane where `Up`/`Down` or `j`/`k` move the selection, `a` adds a todo (offered even when the pane holds none), `Enter` or a row click opens the todo for editing, `Space` toggles done, `g` or the link chip jumps to a linked pane, `d` removes, `c` clears the completed ones, and `Esc`/`q` closes. The edit view cycles priority with `Tab`, opens the link picker with `ctrl+l`, toggles done with `ctrl+t` (the panel's `Space` belongs to the text field there), saves with `ctrl+s`, and cancels with `Esc` — and the priority, link, and done rows are clickable; the done row is absent while composing a new todo, which is never already done; `keys.add_pane_todo` (unset by default) opens it on a new todo. The link picker is the session navigator in a selection mode: every pane in every workspace, searchable, named the way the navigator names them so a plain shell is identified by the command it launched, with a `no link` entry that clears the link and the todo's own pane withheld; workspace and tab rows are context only, and leaving without choosing keeps the previous link. A todo whose linked pane is gone renders dimmed and is inert. `ui.show_pane_todo_indicator` hides the indicator and `ui.pane_todo_color` overrides its color.
- The todo edit view's text field now has an insertion point and the readline chords to move it: `ctrl+a`/`ctrl+e` (line start/end), `ctrl+b`/`ctrl+f` and the arrow keys (character), `alt+b`/`alt+f` and `ctrl+Left`/`ctrl+Right` (word), `ctrl+d` (delete forward), `ctrl+k`/`ctrl+u` (kill to line end/start), `ctrl+w` (kill the word behind the cursor), `ctrl+y` (yank the last kill, from Herdr's own one-entry kill ring rather than the system clipboard), and undo on `ctrl+_`, `ctrl+-`, or `ctrl+/`. Typing and pasting land at the cursor instead of the end, and clicking in the text puts the cursor there rather than cancelling the edit. A todo can also hold more than one line now: `Enter` inserts a newline, paste keeps the newlines in what it pastes (still dropping other control characters), the edit view shows three lines at a time and scrolls to follow the cursor, and the panel keeps one row per todo, showing its first line with a `⏎` marker. **Three keys moved to make room, which is the whole of the muscle-memory change:** `Enter` inserts a newline instead of saving (save is `ctrl+s`, or `alt+Enter` where the terminal reports it), `ctrl+d` deletes forward instead of toggling done (the toggle is `ctrl+t`), and `ctrl+u` kills to the start of the line instead of clearing the whole buffer — which still clears everything whenever the cursor is at the end, where it sits right after typing.
- The link picker's selection now moves under `ctrl+j`/`ctrl+k` in every state, including while its search is focused, where the arrow keys and `ctrl+n`/`ctrl+p` were previously the only way. The same chords work in the ordinary goto picker (`prefix+g`), which is the same surface.
- A live todo link is now shown as `→ w2:pC · claude`: the target's public pane identifier first, because that is the part you can act on with `herdr pane`, `herdr agent read`, or a sibling agent's prompt, and the captured label after it, because that is the part you recognise. The picker's pane rows carry the identifier they would stage, and the edit view's link row leads with it too. The identifier is resolved from the live target every time it is drawn, so it follows a pane that moves rather than going stale; a dead link has no identifier to show and keeps its label alone.
- Following a todo's link is now offered where the link is shown. The panel's footer gained a `go` button beside `add`/`toggle`/`clear done`, advertising the `g` key that already worked but that nothing on screen mentioned, and the edit view gained `ctrl+g` — `ctrl+s` then `g` in one keystroke, committing what you typed and moving focus to the linked pane. Previously the edit view showed you a link with no way to travel to it: `ctrl+l` and a click on the row both re-opened the picker to change it. Both affordances appear only while the link still resolves — a dead link stays inert — and `ctrl+g` saves nothing when there is nowhere to go, so it can never commit an edit and then fail to travel.
- Closing a pane that still has outstanding todos now asks for confirmation, over the socket API as well as in the TUI; a pane whose todos are all done closes without a prompt.
- Added respawn: `keys.respawn_pane` (`prefix+ctrl+x`), the `pane.respawn` socket method, and `herdr pane respawn <pane_id>` restart a pane's process in place, so a wedged agent or dev server can be recovered without closing the pane and re-splitting. The pane keeps its pane id, position, size, label, and todos; only the process is replaced, and the pane's agent runtime identity is cleared so detection re-identifies whatever comes back. A pane launched with a command re-runs that command; a plain shell pane gets a fresh shell, starting in the pane's current directory rather than the one it was launched in. It asks first only when it would kill live work — a running child process or outstanding todos — and respawns immediately otherwise; over the socket the first request then returns `confirmation_required` and repeating it confirms.
- Added `ui.sort_motion_easing`: `"bubble"` eases the bubble-motion step cadence across a reshuffle — slower to break away, quickest mid-flight, slowing into the final slot — instead of the evenly spaced steps of the default `"linear"`. The curve stretches over the reshuffle, so it reads as acceleration only when rows travel several positions; short one-to-three-row moves come out snappier rather than visibly eased.
- Added `ui.workspace_number_prefix` and `ui.agent_number_prefix`: optional leader glyph(s) shown before the editorial jump numbers to hint the jump chord (e.g. `"₽"` renders `₽5` for a workspace jumped by `prefix+5`, `"₽⌥"` renders `₽⌥2` for an agent jumped by `prefix+alt+2`). Rendered in the matching `*_number_color`; only applies when `ui.sidebar_style = "editorial"`. Empty (default) keeps the bare number.
- Added `ui.sidebar_style = "editorial"`: jump numbers move to the right edge of the entry's name row (spaces and agents panels), section headers render as thin uppercase, and inactive meta lines (branch, agent details) dim so names and state icons carry the hierarchy. Default `"default"` keeps the existing layout. Added `[ui.state_colors]` with per-state overrides (`working`, `idle`, `done`, `blocked`, `unknown`) for the sidebar state glyphs and state text — same syntax as `accent`, theme fallback when unset, applied across the expanded sidebar, collapsed sidebar, mobile switcher, and navigator.
- Priority-sorted sidebar lists (spaces with `ui.workspace_sort = "priority"`, and the agents panel in priority sort) now bubble instead of teleporting: when a row's sort position changes — you view a finished agent, or an agent starts working, finishes, or blocks — the row holds its place for `ui.sort_motion_settle_ms` (default 2000), then moves one position per `ui.sort_motion_step_ms` (default 150) until it reaches its slot. State icons and colors still update instantly; only the position travels late, and mouse clicks resolve against the order on screen, so the list no longer reshuffles under the cursor right after you click a done agent. Set `ui.sort_motion = "instant"` to restore immediate re-sorting.
- Added a notification center: the server now keeps a bounded in-memory log (newest 100 entries) of every in-app toast — agent needs-attention and finished notifications, update notifications, and `notification.show` injections — with a per-entry read flag. The tab bar shows a `◆` indicator at its right edge with an unread-count pill; clicking it or pressing `keys.open_notification_center` (`prefix+ctrl+n`) opens a dropdown listing notifications newest-first. Unread entries render with a kind-colored dot and a bold title; visited ones drop the dot and dim. Opening the panel does not touch read state — the count only decreases as notifications are visited. `Up`/`Down` or `j`/`k` move the selection, `Enter` or a row click jumps to the notification's pane (the same focus path as clicking a toast) and marks that entry read, `r` or the footer "mark read" button marks everything read while keeping the history, `c` or the "clear all" button empties the log, and `Esc`/`q` closes. The footer buttons use the settings-panel language: filled boxes with the shortcut hint inside. `ui.notification_center_position` moves the whole affordance to the bottom-right — the `◆` indicator floats in the frame's bottom-right corner and the dropdown opens above it (`"top-right"` default keeps both in the tab bar). The log is exposed as `notification.list` (entries carry `read`) / `notification.mark_seen` (optional `id` marks one entry, no `id` marks all) / `notification.clear` socket methods, a `notification.posted` subscription event, and `herdr notification list [--json]` / `herdr notification clear [--json]`, so external status bars can consume and manage the same feed. The log empties on a cold server restart.
- The scroll gestures gained downward mirrors: `keys.copy_mode_page_down` (`prefix+pagedown`), `keys.copy_mode_half_page_down` (`prefix+ctrl+d`), and `keys.copy_mode_line_down` (`prefix+ctrl+j`). They scroll an open copy mode back down, and on a fullscreen (alt-screen) application they enter the scroll passthrough mode going down. Previously every entry gesture scrolled up, so once the passthrough mode had ended — the prefix chord ends it, which is how you switch space — the only way back to an application still scrolled up was to scroll further up first. On an ordinary pane with copy mode closed they do nothing, since a live viewport has nothing below it.
- Added one-gesture scrollback entry: `keys.copy_mode_page_up` (`prefix+pageup`), `keys.copy_mode_half_page_up` (`prefix+ctrl+u`), and `keys.copy_mode_line_up` (`prefix+ctrl+k`) enter copy mode on the focused pane already scrolled up by a page, half page, or line (tmux `copy-mode -u`). Repeating a gesture scrolls further, and exiting copy mode still restores the pre-entry scroll position. Keybindings now also accept the `pageup` / `pagedown` key names.
...
- `keys.move_pane_to_tab` (`prefix+m`) now moves a pane anywhere in the session instead of only within its own space. The picker lists the other tabs of the pane's own space first, then the tabs of every other space, plus a `new tab` destination in any listed space and a `new space` destination last that creates the space as part of the move — unnamed, exactly like a space created any other way, and renameable afterwards. Destinations are grouped under a space heading that is a label rather than a selectable row, so the selection and the mouse both step over it. Picking a destination in another space switches to that space with the moved pane focused, and the pane keeps its running process and terminal contents either way; moving the last pane of a tab leaves no empty tab behind. The picker no longer refuses to open merely because the current space has a single tab — it is now suppressed only when the pane is the session's only pane, in its only tab, in its only space. `prefix+!`, `prefix+>` and `prefix+<` keep their current tab-scoped meaning.
- Added a session-wide todo board: `keys.open_todo_board` (unset by default) opens one view of every pane's todos, so "what is outstanding in this session" no longer means visiting panes one at a time. Todos are grouped by their owning pane in space, tab, then pane order, each group headed by the pane's addressable identifier and label (`w2:pC · claude`); panes holding nothing are omitted, and todos are deliberately not interleaved into one priority-ordered list, so a pane's todos stay contiguous. Its keys are the pane todo panel's — `Space` toggles done, `g` or the link chip follows a linked todo to its *linked* pane, `d` removes, `c` clears the completed todos of the selected todo's pane, `Esc`/`q` closes — with one difference: `Enter` (or a row click) focuses the pane that *owns* the todo and closes the board, which is what the board offers over `herdr todo list --all`, so editing is on `e`. Group headings are never selectable. The board opens even when nothing is outstanding, and it complements the per-pane panel rather than replacing it: both read the same store, so a todo looks and sorts the same in either.
- The tab bar now shows a todo indicator in its trailing corner, immediately left of the notification indicator: the session-wide outstanding count, colored by the highest outstanding priority (the same rule as the per-pane `▾` border indicator), as a bare glyph when nothing is outstanding. Clicking it toggles the session todo board — the board's first mouse entry.

### Changed
- The todo board's selection, pointer and visible window are now three separate things. Moving the pointer no longer drags the selection across the rows it crosses, and the scroll wheel moves the window instead of the selection. A click on a row selects it and stops; a second click on the already-selected row travels to its pane, so travel is still mouse-only but a stray click while scanning can no longer close the board and throw you into another space. A todo's link chip still follows its link on the first click — it is an explicit target rather than a row. The pane todo panel and the notification centre are unchanged: they act in place, so a stray click there costs nothing.
- The tab bar's trailing indicators now speak a modified-letter glyph language: `τ` for todos and `и` (a mirrored N) for notifications, replacing the notification `◆`. The sidebar's stroked prefix badge set the precedent — a letter with a twist reads as a name, not an icon set. The per-pane todo indicator keeps `▾` as its handle — it marks a place on a pane, not an entry point in the chrome — and names its count the same way, `▾ τ N`, with the count in bold while anything is outstanding. An unread notification count is now accent text like the todo count beside it, not an inverse pill.
- Sidebar agent rows show each pane's open todo count, `τ N`, at the right end of the entry's name row, coloured by the highest open priority, with the editorial jump label moved under it on the second row; the count disappears when nothing is open, and a one-row entry keeps only its jump label.
- Titled overlays now leave a blank row between the title and their first content row, the same rule `FOOTER_ROWS` already gave the buttons at the other end. The keybind help panel, the rename dialog, the todo edit modal and the release-notes header already did this; the todo board, the pane-move picker and the new/open/remove worktree dialogs did not, and now do. Each of those five is one row taller.
- The todo board's group headings now read `space · pane label` (`infra · imap-jmap-mcp`) instead of leading with the pane's addressable identifier. The board is read across spaces, so where the work lives is the first thing a heading should answer, and the space was previously only encoded in the first character of an identifier you had to decode. The identifier is dropped rather than moved: `w5:pV` is a creation counter in a 32-character alphabet, so it says the pane was the 27th ever opened in that space and nothing about which pane is in front of you, and activating a row travels there without anyone reading an address. A heading omits either part when the pane does not have it rather than rendering it empty. The todo link chip still leads with its identifier: a chip is a destination you may want to address with `herdr pane` or `herdr agent read`, and it shares its row with the todo's own text and is truncated from the right, so there the part you can act on is the part that has to survive.
- The todo board now separates its groups: a blank row sits between each pane's group and the one above it, and a group's todos are indented under its heading. Dogfooded against eleven panes across ten spaces, the old flush list read as one twenty-two-row block that had to be re-parsed line by line to see which row was a heading. The blank row is a row of the list rather than an offset the renderer adds, so scrolling, selection and clicks all still agree on which item a row is; it never appears above the first group, is never selectable, and a click on it does nothing. Indentation narrows the rect a todo is drawn into rather than changing the row renderer, so a todo still reads identically on the board and on the pane's own panel.
- The todo board is wider: the floor rises from 64 to 80 columns and the cap from 120 to 140, with the content still deciding between them. A session whose longest row was 66 columns previously got a 66-column box in the middle of a very wide terminal, truncating todos at a width the screen had no need to impose.
- The todo board is now titled `todos/notes`, and the overlay that composes or edits an entry is titled `new todo/note` and `edit todo/note` to match. What a pane records is as often a note to self as a task, and a title naming only one of them says the other does not belong there. Only the title changes: `herdr todo`, `keys.open_todo_board`, the socket API and the protocol keep their names, because those are addresses typed into scripts and configs rather than words the user reads while working.
- The todo compose/edit overlay grew from a 60-column modal with a three-row text block to 84 columns with eight rows, and the text block now soft-wraps at word boundaries instead of scrolling horizontally. Explicit newlines are preserved as hard breaks, the caret stays visible by vertical scroll through the wrapped rows, and a click lands on the clicked character through the same wrap layout the renderer used. Wrapping is presentation only: the stored text is unchanged. At the 500-character cap a single unbroken paragraph wraps to about seven rows, so a whole typical todo is visible at once.
- Every todo row on the panel and the board now shows the todo's id, dim and right-aligned as `#12` — the same id `herdr todo done/edit/rm` accepts, so a todo can be named in conversation: by the user to an agent, and by one agent to another (its full cross-pane address is the owning pane plus the id). The edit overlay's title names the id it is editing (`edit todo/note #12`); composing a new todo shows no id because none exists until it is saved. Both surfaces widen for the id rather than giving up text room, and ids line up in one scannable column whether or not a row carries a link chip.
- The todo board now sizes itself to its content instead of a fixed box, so a session with todos on several panes shows them all rather than scrolling immediately. It widens for its longest heading or todo (up to 120 columns) and grows as tall as its rows need, clamped by the terminal in both directions, and keeps a minimum size so an empty board still reads as a panel.
- Every list-bearing overlay now accepts the same chords to move its selection: the arrow keys, `j`/`k`, `ctrl+j`/`ctrl+k`, `ctrl+n`/`ctrl+p`, half a page on `ctrl+u`/`ctrl+d`, and first/last on `Home`/`End`. That is a superset almost everywhere — the notification center, the todo panel, the settings sections, the global and context menus, the pane-move picker and the keybind help panel all gained chords they did not have — with two deliberate exceptions. The todo panel's letter actions now require no modifier, so `ctrl+d` there is half a page down rather than a second way to spell "remove" (and `ctrl+c` no longer clears the notification log); and the two-item settings sections clamp instead of flipping, which is the rule every other list follows. In an overlay whose search box has focus, plain `j`/`k` are still text and `ctrl+u`/`ctrl+d` still belong to the text field.
- The notification center and the pane todo panel now scroll by the minimum needed to keep the selection visible, and leave the window alone when the selection is already in it. Their window used to be derived from the selection alone, which scrolled the list by one every time the selection moved up even while it was still on screen.
- The rename inputs (workspace, tab, pane), the worktree-create branch field, and the navigator and keybind-help search boxes now have an insertion point and the same readline editing the todo edit view has: `ctrl+a`/`ctrl+e`, `ctrl+b`/`ctrl+f`, `alt+b`/`alt+f`, `ctrl+Left`/`ctrl+Right`, `ctrl+k`/`ctrl+u`, `ctrl+w`, `ctrl+y`, `ctrl+d`, undo, and one definition of where a word ends. Typing and pasting land at the cursor rather than always at the end, and the caret is drawn where it is. One chord changed meaning: `ctrl+h` in a rename input now deletes a character rather than a word, because many terminals send `ctrl+h` for Backspace itself and word-deleting on it made Backspace eat a whole word on those terminals.
...
- Bumped the client/server protocol version to 22. Upstream also landed on 21 from the same base, so the fork moves one past it; the fork's 21 carried `notification.*`, `todo.*`, `layout.balance`, `layout.set_preset` and `pane.respawn`, which upstream's does not.
...
- A session file that cannot be read is now preserved instead of being silently overwritten. Previously a truncated `session.json` — the shape a power loss can leave behind — started an empty session and then, seconds later, wrote that empty session over the damaged file, destroying the only remaining copy of the user's panes, working directories and todos. herdr now moves an unusable session file aside under a UTC-timestamped name (`session.20260820-090944Z.bak.json`) before anything else, logs that it did, and starts empty as before. This also covers a session written by a newer herdr than the one starting, which was previously declined and then overwritten — so running an older binary once destroyed the newer session it had politely refused to read. Preserved files are never deleted automatically. Session writes now also flush to disk before the rename that publishes them, closing the window in which a power loss makes the rename durable before the data.
- The todo board's first group heading is no longer stranded once the list has scrolled. Scrolling was driven entirely by keeping the *selection* visible, and the selection can only land on a todo — never on a heading — so the scroll offset could never reach zero and the first heading was unreachable by keyboard or wheel. Selection-driven scrolling now keeps the selected todo's heading on screen too whenever both fit, so a selected row always says which pane it belongs to.
- Held keys now auto-repeat inside overlays. Holding an arrow (or any key) in the todo editor, the todo board and panel, the navigator and keybind-help search boxes, the rename and worktree dialogs, the notification center — any overlay — delivered only the initial press, so moving ten rows meant ten keystrokes. Overlay modes now hold a stable input context the way copy mode and app-scroll already did, so a held key repeats until the press that closes or switches the overlay, at which point its remaining repeats stop rather than replaying into whatever opened.
- Agent panes with no assigned name are no longer all called `claude` in the session navigator and the todo board. A pane's label preferred the agent's own reported title, which carries a TTL and goes quiet once the pane has been idle for a while, and nothing below it in the chain consulted the terminal's own title — so a pane whose session was plainly named `infra-side-of-direction-on-kube` on screen appeared as `claude` in exactly the lists you use to find it. The terminal title now sits between an assigned name and the bare agent name: a name given with `herdr agent name` still wins, because that is what `herdr agent send` addresses.
- The todo board's `clear done` now clears the completed todos of every pane the board is showing, not just the selected todo's pane. Scoped to the selection it did nothing — and said nothing — whenever the selection was not on a pane with completed todos, which is the common case since the board opens on the first todo in the list; the footer names no scope, and the board is the session's view, so neither should the action. The per-pane panel's `c` is unchanged and still clears that pane alone.
- The notification center's `mark read` button is now always on its footer. The panel's minimum width was 30 columns and the three-button footer wants 41, so on anything but a long notification title the button was dropped — hiding the action *and* the `r` hint that was the only other way to reach it. The panel now measures its own footer alongside its rows, so it is never narrower than the controls it means to show. An empty panel, which has no footer, keeps its narrow box.
- Panes no longer inherit `SSH_CLIENT`, `SSH_CONNECTION`, and `SSH_TTY` from the environment the server was started in. The server outlives that connection — it daemonizes, keeps running after the ssh session closes, and carries the same environment across every handoff — so a server first started over ssh left every pane it spawned looking like it was running on a connection that no longer existed, to prompts, shell rc files, and anything else that branches on ssh. The vars never described the pane's own client either, since any number of local and remote clients may be attached. `SSH_AUTH_SOCK` is untouched, so forwarded agents still reach panes.
- An agent running behind a PTY wrapper is now identified instead of being invisible. Agent identification reads the foreground job of the pane's own PTY, so a wrapper that allocates a PTY of its own and runs the shell inside it — Atuin's `pty-proxy`, shipped in v18.13 and enabled by its init snippet, is the one this happens with — left the pane's PTY carrying only the wrapper and the agent one level down, out of reach. The pane was then not an agent as far as Herdr was concerned: absent from the agents sidebar and `agent.list`, unresolvable by `agent.get`, `agent.read`, `agent.send`, `agent.wait`, and `agent.explain`, contributing no state to its workspace, and reporting the wrapper's working directory as its own. Nothing reported the gap — the pane simply looked like a plain shell printing an agent's interface, and because a wrapper wraps every shell it initialises, every agent pane opened after enabling one was affected while older panes kept working. When a pane's own foreground job yields no agent and its process group leader is a recognised wrapper, identification now descends one level into the PTY that wrapper owns, and the agent, its process name, and the pane's reported working directory all come from the job it was identified in. Recognition is an explicit set of wrapper process names, currently `atuin` alone, rather than a walk of the process tree: a wrapper the set does not name behaves exactly as before, so an unrecognised one yields no agent instead of a wrong one. Unwrapped panes take an identical path to before, and the lookup rides the existing foreground-job probe rather than sampling on its own.
- Panes no longer inherit `ATUIN_PTY_PROXY_ACTIVE` and `ATUIN_PTY_PROXY_TMUX` either, for the same reason. Atuin's PTY proxy sets them so a shell re-running its rc inside the proxy does not wrap itself twice, but inherited from the server's environment they describe a proxy sitting between some other shell and its terminal, not between a pane's shell and that pane's PTY. A pane that inherited them skipped wrapping, and Atuin's OSC 133 output capture silently never recorded there. Each pane's shell now decides for itself, the same way Atuin already re-wraps per tmux pane. The rest of Atuin's environment, `ATUIN_SESSION` included, is untouched.
...
- A sidebar row covering several panes no longer shows `done` while an agent under it is working. Workspace, tab, and collapsed worktree-group rows all took their state from the attention ranking, which deliberately puts a finished-but-unviewed agent above a working one — so one agent that finished while you were elsewhere masked every working sibling under that row, and the row only flipped to `working` once you switched into the space, because the switch marked the finished pane seen. It was the switch that revealed the working agent, not anything the agent did. Rolled-up rows now rank `blocked`, `working`, `done`, `idle`, `unknown`, and a row's state depends only on the panes it covers, so viewing one pane never changes what another row shows. Priority sorting is unchanged and still surfaces finished agents above working ones, which is the point of it. `workspace.info` and `tab.info` report the same rollup on the JSON API, so they no longer answer `done` for a container whose agent is working; per-pane `agent_status` never rolled up and is untouched.
- Modal titles no longer sit against the frame. Every modal indents its rows one column off the border, but the title was drawn in the frame's first inner column, so `new todo`, `keybinds`, `new worktree` and the rest read as stuck to the edge while everything under them was inset. The title now shares the rows' column. The release-notes and product-announcement headers, which had each grown their own one-column indent, drop it so they stay level with the other modals rather than moving a column further in.
- Indexed jump ranges (`focus_agent = ["prefix+alt+1..9", "prefix+alt+a..z"]` and friends) now skip `e`, `i`, `n`, and `u` on macOS. Those are Option dead keys on ABC and most Latin layouts, so Option+e emits `´` and Option+i emits nothing at all instead of `alt+e`/`alt+i` — the keypress never reaches Herdr, and Ghostty's `macos-option-as-alt` does not change it (it defeats the plain Option translation, not dead-key composition). Previously the sidebar advertised `⌥e` on entry 14 and pressing it silently did nothing, with entries 18, 23, and 30 equally unreachable. The jump label and the keypress matcher now both derive from one filtered sequence, so a label can never name a letter matching refuses; on macOS entry 14 is `⌥f`. Linux and Windows keep the full `a..z`. An out-of-range agent jump also logs at debug level instead of failing silently.
- The collapsed sidebar now highlights the active agent (the same background band the active space gets, with the jump symbol bold in the text color — the active space symbol is bold too), labels entries past the 9th with the real jump letters (`a`, `b`, …) instead of two-digit numbers that named non-existent chords and collided with the state icon, honours `ui.workspace_number_color` / `ui.agent_number_color`, and honours `ui.sidebar_active_border = "left"`/`"right"` by reserving one extra edge column and drawing the accent bar on the active space and agent rows (`above`/`below`/`both` still draw nothing in collapsed mode, which has no spacer rows).
- `--remote` no longer reinstalls and offers to stop a remote server that is already running the same build. Remote binary discovery now looks for the name the client was invoked as (e.g. `herdr-beta`) before falling back to `herdr`, across `command -v` and every known install path, and installs under that same name (`~/.local/bin/<name>`). Previously discovery hardcoded `herdr`, so on a machine with both channels installed it found the stable `herdr`, saw a version mismatch against the beta client, and fell through to install — prompting to stop the running remote server even though both sides were fully up to date.
- `keys.next_layout` (cycle tab layout presets) and `keys.balance_panes` are now honored from `config.toml`. Both actions shipped in the runtime and in `--default-config`, but were missing from the config deserialize overlay, so rebinding them was silently ignored — and once the unknown-key reporting landed, they were flagged as unknown `[keys]` entries. A guard test now fails if any keybinding action is present in the serialize config but absent from the deserialize overlay.
