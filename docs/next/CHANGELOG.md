...
- Added respawn: `keys.respawn_pane` (`prefix+ctrl+x`), the `pane.respawn` socket method, and `herdr pane respawn <pane_id>` restart a pane's process in place, so a wedged agent or dev server can be recovered without closing the pane and re-splitting. The pane keeps its pane id, position, size, label, and todos; only the process is replaced, and the pane's agent runtime identity is cleared so detection re-identifies whatever comes back. A pane launched with a command re-runs that command; a plain shell pane gets a fresh shell. It asks first only when it would kill live work — a running child process or outstanding todos — and respawns immediately otherwise; over the socket the first request then returns `confirmation_required` and repeating it confirms.
