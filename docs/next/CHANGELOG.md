...
- The todo board's group headings now read `space · pane label [addressable id]` (`infra · imap-jmap-mcp [w2:pP]`) instead of leading with the identifier. The board is read across spaces, so where the work lives is the first thing a heading should answer, and the space was previously only encoded in the first character of an identifier you had to decode. A heading omits any part it cannot resolve rather than rendering it empty. The todo link chip still leads with the identifier: it shares a row with the todo's own text and is truncated from the right, so there the part you can act on is the part that has to survive.
