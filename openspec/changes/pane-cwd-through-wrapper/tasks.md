...
- [ ] 1.1 A pure resolver in `src/pane.rs` taking the pane's child pid plus
...
- [ ] 1.2 Tests: leader wins over an earlier member; falls back to a member;
      falls back to the direct child when there is no nested job; `None` when
      nothing can answer

## 2. Publish it from the detection task

- [ ] 2.1 A resolved-cwd cell on `PaneRuntime`, filled by the per-pane detection
      task on its own fixed interval, independent of the agent-probe throttle
- [ ] 2.2 `PaneRuntime::cwd` reads reported, then resolved, then the direct
      child; the walk never runs behind `cwd`
- [ ] 2.3 Tests: `cwd` prefers a reported directory over a resolved one, and a
...
- [ ] 3.1 Delete `PaneRuntime::interactive_cwd` and its `TerminalRuntime`
      wrapper; point `respawn_pane_runtime` at `cwd`
- [ ] 3.2 Keep the respawn cwd test passing against `cwd`
