...
- [ ] 1.1 A direction (up | down) threaded through
...
- [ ] 1.2 Downward on a primary-screen pane with no active copy mode does
      nothing: no copy-mode entry, no mode change
- [ ] 1.3 Tests: down on an alt-screen pane enters the passthrough and sends
...
- [ ] 2.1 `KeysConfig.copy_mode_page_down` / `copy_mode_half_page_down` /
...
- [ ] 2.2 The new actions join the set of prefix actions that do not cancel
      active copy mode, like their upward counterparts
- [ ] 2.3 Tests: each default chord resolves to its action; the actions do not
      cancel copy mode; the help panel lists them
- [ ] 2.4 Docs: keyboard page and the unreleased changelog
