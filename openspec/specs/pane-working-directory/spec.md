# pane-working-directory Specification

## Purpose
Define the working directory Herdr reports for a pane: the directory of the
shell the user is interacting with, including through a wrapper process that
re-runs that shell inside a nested PTY, the order the sources are trusted in,
the background cadence that resolves it so no consumer pays a process walk on a
presentation path, and the requirement that every surface derived from it
agrees.
