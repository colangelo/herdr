...
## Purpose
Define the server-owned notification center: a bounded in-memory log of every
in-app toast with seen-marker semantics, exposed through the socket API
(`notification.list` / `notification.mark_seen` / `notification.clear`), a
`notification.posted` subscription event, and the CLI, and presented in the
TUI as an unread indicator (tab-bar or floating bottom-right, per
`ui.notification_center_position`) with a keyboard-navigable dropdown that
jumps to notification targets and can clear the log.
