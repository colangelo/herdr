use crate::api::schema::{
    LayoutBalanceParams, LayoutPreset, LayoutSetPresetParams, Method, PaneCurrentParams,
    PaneDirection, PaneEdgesParams, PaneFocusDirectionParams, PaneLayoutParams, PaneListParams,
...
fn pane_layout(args: &[String]) -> std::io::Result<i32> {
    ...
    let pane_id = match parse_optional_current_pane_args(&rest) {
        Ok(pane_id) => pane_id,
        Err(message) => {
            eprintln!("{message}");
            return Ok(2);
