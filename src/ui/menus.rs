...
pub(super) fn render_app_scroll_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    let key = Style::default()
        .fg(app.palette.accent)
        .add_modifier(Modifier::BOLD);
    let dim = Style::default().fg(app.palette.overlay0);
    ...
    let line = Line::from(vec![
        ...
        Span::styled("keys scroll the app", dim),
    ]);
...
pub(super) fn render_navigate_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let balance = prefix_rhs_label(&kb.balance_panes);
    let next_layout = prefix_rhs_label(&kb.next_layout);
    ...
    let line = Line::from(vec![
        ...
        Span::styled(balance, key),
        Span::styled(" balance  ", dim),
        Span::styled(next_layout, key),
